/*
Update your simulation with CrateMover 9001!
*/
...
int main()
{
    /* Strategy
    Array of stack.
    Place the items to be removed in a temporary stack and take the items
    from there to place on the destination stack. (first out -> last in)
    */
    ...
    while ((in >> char1 >> char1 >> char1 >> char1 >> num >> char1 >> char1 >> char1 >> char1 >> strId >> char1 >> char1 >> dstId))
    {
        ...
        for (int p = 0; p < num; ++p)
        {
            ...
            placeHolder.push(removedItem);
        }
        for (int p = 0; p < num; ++p)
        {
            removedItem = placeHolder.top();
            placeHolder.pop();
